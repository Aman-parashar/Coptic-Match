...
//        }
        ...
        if audioSession.isActive == false {
            ...
            // Initialize audio session with new configuration
            ...
            // Use new setConfiguration method instead of initialize with closure
...
extension CallKitManager: CXProviderDelegate {
    ...
    func provider(_ provider: CXProvider, didDeactivate audioSession: AVAudioSession){
        ...
        let rtcAudioSession = QBRTCAudioSession.instance()
        if rtcAudioSession.isActive == true {
            //debugPrint("Deinitializing session in CallKit callback.")
            rtcAudioSession.setActive(false)
        }
    }
}
