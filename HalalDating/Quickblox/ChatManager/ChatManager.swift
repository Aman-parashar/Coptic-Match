...
struct ChatManagerConstant {
    static let messagesLimitPerDialog = 100000
    static let chatServiceDomain = "com.q-municate.chatservice"
    static let errorDomaimCode = -1000
    static let notFound = "SA_STR_DIALOG_REMOVED"
}

struct DialogsConstant {
    static let dialogsPageLimit:Int = 100000
...
class ChatManager: NSObject {
    ...
    func updateStorage() {
        ...
        if QBChat.instance.isConnected == false {
            self.delegate?.chatManager(self, didFailUpdateStorage: "SA_STR_NETWORK_ERROR")
            return
        }
        ...
        loadGroup.notify(queue: DispatchQueue.main) {
            if message.isEmpty {
                self.delegate?.chatManager(self, didUpdateStorage: "SA_STR_COMPLETED")
            } else {
    ...
    //MARK: - Users
    func loadUser_email(_ email: String, completion: ((QBUUser?) -> Void)? = nil) {
        QBRequest.user(withEmail: email, successBlock: { (response, user) in
    ...
    func loadMultipleUser(_ id: [String], completion: (([QBUUser]?) -> Void)? = nil) {
        QBRequest.users(withIDs: id, page: QBGeneralResponsePage(currentPage: UInt(1), perPage: 1000000), successBlock: { (response, page, users) in
...
//        }
        ...
        message.senderID = select_QBUserForChat.id
        message.text = text
        message.markable = true
        message.deliveredIDs = [(NSNumber(value: select_QBUserForChat.id))]
        message.readIDs = [(NSNumber(value: select_QBUserForChat.id))]
        ...
        systemMessage.senderID = select_QBUserForChat.id
        systemMessage.markable = false
        systemMessage.text = text
        systemMessage.deliveredIDs = [(NSNumber(value: select_QBUserForChat.id))]
        systemMessage.readIDs = [(NSNumber(value: select_QBUserForChat.id))]
        ...
        let systemMessagesOperation = BlockOperation {
            for occupantID in occupantIDs {
                if select_QBUserForChat.id == occupantID.intValue {
                    continue
                }
    ...
    func createGroupDialog(withName name: String,
        ...
        QBRequest.createDialog(chatDialog, successBlock: { response, dialog in
            dialog.join(completionBlock: { (error) in
                ...
                self.delegate?.chatManager(self, didUpdateStorage: "SA_STR_CREATE_NEW" + dialogName)
...
//                return
//            }
            let currentUserID = QBSession.current.currentUserID
            let dialog = QBChatDialog(dialogID: nil, type: .private)
            dialog.occupantIDs = [NSNumber(value: currentUserID), NSNumber(value:opponent.id)]
...
//        }
        if QBChat.instance.isConnected == true {
            ...
            QBChat.instance.connect(withUserID: select_QBUserForChat.id,
                                    password: Validation.LoginConstant.defaultPassword,
    ...
    private func errorMessage(response: QBResponse) -> String? {
        ...
        if response.status.rawValue == 502 {
            errorMessage = "SA_STR_BAD_GATEWAY"
        } else if response.status.rawValue == 0 {
            errorMessage = "SA_STR_NETWORK_ERROR"
        } else {
