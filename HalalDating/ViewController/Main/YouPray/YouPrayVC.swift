...
class YouPrayVC: UIViewController {
    //MARK: - @IBOutlet
    ...
    var arrList = ["Always pray".localizableString(lang: Helper.shared.strLanguage),
     "Usually pray".localizableString(lang: Helper.shared.strLanguage),
     "Sometimes pray".localizableString(lang: Helper.shared.strLanguage),
     "Never pray".localizableString(lang: Helper.shared.strLanguage)]
    var selectIndex = 0
    ...
    override func viewDidLoad() {
        ...
        setProgress()
        registerCell()
        AppHelper.updateScreenNumber(userID: userModel.data?.id ?? 0, screenNumber: 15, token: userModel.data?.api_token ?? "")
    ...
    func setLanguageUI() {
        ...
        lblTitle.text = "How often do you pray?".localizableString(lang: Helper.shared.strLanguage)
    }
    func setHeaderView(){
        headerView.delegate_HeaderView = self
...
//            if title.lowercased() == "\(userModel.data?.pray ?? "")".lowercased(){
//                selectIndex = index
//            }
//        }
        
        if (userModel.data?.pray ?? "") == "Always pray" || (userModel.data?.pray ?? "") == "دائما اصلى" {
            selectIndex = 0
            ...
            selectIndex = 2
        } else if (userModel.data?.pray ?? "") == "Never pray" || (userModel.data?.pray ?? "") == "لا اصلى" {
            selectIndex = 3
        }
    ...
    @IBAction func btnNext(_ sender: Any) {
        //--
        userModel.data?.pray = arrList[selectIndex]
        if isComeProfile{
...
extension YouPrayVC: UITableViewDelegate, UITableViewDataSource
{
...
extension YouPrayVC: HeaderView_Protocol{
    func btnBack() {
        self.navigationController?.popViewController(animated: true)
    }
}
