...
//    var arrList = ["Leo", "Virgo", "Libra", "Scorpio", "Sagittarius", "Capricorn", "Aquarius", "Pisces"]
    var arrList = ["Aries", "Taurus", "Gemini", "Cancer", "Leo", "Virgo", "Libra", "Scorpio", "Sagittarius", "Capricorn", "Aquarius", "Pisces"]
    ...
    override func viewDidLoad() {
        ...
        setProgress()
        registerCell()
        AppHelper.updateScreenNumber(userID: userModel.data?.id ?? 0, screenNumber: 17, token: userModel.data?.api_token ?? "")
    ...
    func setLanguageUI() {
        ...
        lblTitle.text = "What's your zodiac sign?"
    }
    func setHeaderView(){
        headerView.delegate_HeaderView = self
    ...
    @IBAction func btnNext(_ sender: Any) {
        //--
        userModel.data?.your_zodiac = arrList[selectIndex]
        if isComeProfile{
...
extension ZodiacVC: UITableViewDelegate, UITableViewDataSource
{
...
extension ZodiacVC: HeaderView_Protocol{
    func btnBack() {
        self.navigationController?.popViewController(animated: true)
    }
}
