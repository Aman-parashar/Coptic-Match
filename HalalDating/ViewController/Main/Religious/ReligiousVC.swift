...
class ReligiousVC: UIViewController {
    //MARK: - @IBOutlet
    ...
    var arrList = ["Practising".localizableString(lang: Helper.shared.strLanguage),
     "Very practising".localizableString(lang: Helper.shared.strLanguage),
     "Moderately practising".localizableString(lang: Helper.shared.strLanguage),
     "Not Practising".localizableString(lang: Helper.shared.strLanguage)]
    ...
    override func viewDidLoad() {
        ...
        setProgress()
        registerCell()
        AppHelper.updateScreenNumber(userID: userModel.data?.id ?? 0, screenNumber: 14, token: userModel.data?.api_token ?? "")
    ...
    func setLanguageUI() {
        ...
        lblTitle.text = "How religious are you?".localizableString(lang: Helper.shared.strLanguage)
    }
    func setHeaderView(){
        headerView.delegate_HeaderView = self
    ...
    func setProgress(){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
          UIView.animate(withDuration: 1, delay: 0, options: [.beginFromCurrentState, .allowUserInteraction], animations: { [unowned self] in
              ...
              let percentProgress = Float(11.0/16.0)
...
//            if title.lowercased() == "\(userModel.data?.religious ?? "")".lowercased(){
//                selectIndex = index
//            }
//        }
        
        if (userModel.data?.religious ?? "") == "Practising" || (userModel.data?.religious ?? "") == "امارس" {
            selectIndex = 0
            ...
            selectIndex = 2
        } else if (userModel.data?.religious ?? "") == "Not Practising" || (userModel.data?.religious ?? "") == "لا امارس" {
            selectIndex = 3
        }
    ...
    @IBAction func btnNext(_ sender: Any) {
        ...
        //--
        userModel.data?.religious = arrList[selectIndex]
        if isComeProfile{
...
extension ReligiousVC: UITableViewDelegate, UITableViewDataSource
{
...
extension ReligiousVC: HeaderView_Protocol{
    func btnBack() {
        self.navigationController?.popViewController(animated: true)
    }
}
