...
class WorkOutVC: UIViewController {
    ...
    var arrList = ["Active".localizableString(lang: Helper.shared.strLanguage), "Sometimes".localizableString(lang: Helper.shared.strLanguage), "Almost never".localizableString(lang: Helper.shared.strLanguage)]
    var selectIndex = 0
    ...
    override func viewDidLoad() {
        ...
        setProgress()
        registerCell()
        AppHelper.updateScreenNumber(userID: userModel.data?.id ?? 0, screenNumber: 12, token: userModel.data?.api_token ?? "")
    ...
    func setLanguageUI() {
        ...
        lblTitle.text = "Do you work out?".localizableString(lang: Helper.shared.strLanguage)
    }
    func setHeaderView(){
        headerView.delegate_HeaderView = self
    ...
    func setProgress(){
        let percentProgress = Float(8.0/16.0)
...
//            if title.lowercased() == "\(userModel.data?.work_out ?? "")".lowercased(){
//                selectIndex = index
//            }
//        }
    ...
    @IBAction func btnNext(_ sender: Any) {
        //--
        userModel.data?.work_out = arrList[selectIndex]
        if isComeProfile{
...
extension WorkOutVC: HeaderView_Protocol{
    func btnBack() {
        self.navigationController?.popViewController(animated: true)
    }
}
