...
class EatHalalVC: UIViewController {
    //MARK: - @IBOutlet
    ...
    var arrList = ["Yes".localizableString(lang: Helper.shared.strLanguage), "No".localizableString(lang: Helper.shared.strLanguage)]
    var selectIndex = 0
    ...
    func setLanguageUI() {
        ...
        lblTitle.text = "Do you only eat halal?".localizableString(lang: Helper.shared.strLanguage)
    }
    func setHeaderView(){
        headerView.delegate_HeaderView = self
...
//            if title.lowercased() == "\(userModel.data?.eating ?? "")".lowercased(){
//                selectIndex = index
//            }
//        }
    ...
    @IBAction func btnNext(_ sender: Any) {
        //--
        userModel.data?.eating = arrList[selectIndex]
        if isComeProfile{
...
extension EatHalalVC: UITableViewDelegate, UITableViewDataSource
{
...
extension EatHalalVC: HeaderView_Protocol{
    func btnBack() {
        self.navigationController?.popViewController(animated: true)
    }
}
