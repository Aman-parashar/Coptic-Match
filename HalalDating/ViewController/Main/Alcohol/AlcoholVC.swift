...
class AlcoholVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        setProgress()
        registerCell()
        AppHelper.updateScreenNumber(userID: userModel.data?.id ?? 0, screenNumber: 19, token: userModel.data?.api_token ?? "")
    ...
    func setLanguageUI() {
        ...
        lblTitle.text = "Do you drink alcohol?".localizableString(lang: Helper.shared.strLanguage)
    }
    func setHeaderView(){
        headerView.delegate_HeaderView = self
...
//            if title.lowercased() == "\(userModel.data?.alcohol ?? "")".lowercased(){
//                selectIndex = index
//            }
//        }
    ...
    @IBAction func btnNext(_ sender: Any) {
        //--
        userModel.data?.alcohol = arrList[selectIndex]
        if isComeProfile{
...
extension AlcoholVC: UITableViewDelegate, UITableViewDataSource
{
...
extension AlcoholVC: HeaderView_Protocol{
    func btnBack() {
        self.navigationController?.popViewController(animated: true)
    }
}
