...
class HaveKidsVC: UIViewController {
    //MARK: - @IBOutlet
    ...
    var arrList = ["Have kids".localizableString(lang: Helper.shared.strLanguage), "Don't have kids".localizableString(lang: Helper.shared.strLanguage)]
    var selectIndex = 0
    ...
    override func viewDidLoad() {
        ...
        setProgress()
        registerCell()
        AppHelper.updateScreenNumber(userID: userModel.data?.id ?? 0, screenNumber: 8, token: userModel.data?.api_token ?? "")
    ...
    func setLanguageUI() {
        ...
        lblTitle.text = "Do you have kids?".localizableString(lang: Helper.shared.strLanguage)
    }
    func setHeaderView(){
        headerView.delegate_HeaderView = self
...
//            if title.lowercased() == "\(userModel.data?.kids ?? "")".lowercased(){
//                selectIndex = index
//            }
//        }
    ...
    @IBAction func btnNext(_ sender: Any) {
        //--
        userModel.data?.kids = arrList[selectIndex]
...
extension HaveKidsVC: UITableViewDelegate, UITableViewDataSource
{
...
extension HaveKidsVC: HeaderView_Protocol{
    func btnBack() {
        self.navigationController?.popViewController(animated: true)
    }
}
