...
class LookingForVC: UIViewController {
    ...
    var arrList = ["Marriage", "Serious Relationship"]
    var selectIndex = 0
    ...
    override func viewDidLoad() {
        ...
        setProgress()
        registerCell()
        AppHelper.updateScreenNumber(userID: userModel.data?.id ?? 0, screenNumber: 11, token: userModel.data?.api_token ?? "")
    ...
    func setLanguageUI() {
        ...
        lblTitle.text = "What you looking for?".localizableString(lang: Helper.shared.strLanguage)
    }
    func setHeaderView(){
        headerView.delegate_HeaderView = self
...
//            if title.lowercased() == "\(userModel.data?.looking_for ?? "")".lowercased(){
//                selectIndex = index
//            }
//        }
        ...
        if (userModel.data?.looking_for ?? "") == "Marriage" || (userModel.data?.looking_for ?? "") == "الزواج" {
            selectIndex = 0
        } else if (userModel.data?.looking_for ?? "") == "Serious Relationship" {
            selectIndex = 1
        }
        tblList.reloadData()
    }
    ...
    @IBAction func btnNext(_ sender: Any) {
        ...
        //--
        userModel.data?.looking_for = arrList[selectIndex]
        if isComeProfile{
...
extension LookingForVC: UITableViewDelegate, UITableViewDataSource
{
...
extension LookingForVC: HeaderView_Protocol{
    func btnBack() {
        self.navigationController?.popViewController(animated: true)
    }
}
